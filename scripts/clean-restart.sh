...
if [ "$KEEP_DATA" = false ]; then
    docker compose -f docker-compose.yml down -v --remove-orphans 2>&1 | grep -v "^$" || true
    print_success "Containers, volumes, and networks removed"
else
    docker compose -f docker-compose.yml down --remove-orphans 2>&1 | grep -v "^$" || true
...
cd "$BASE_DIR/infrastructure/docker"
docker compose -f docker-compose.yml up -d 2>&1 | grep -v "^$" | head -20 || true
cd "$BASE_DIR"
