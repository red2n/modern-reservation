...
-- Date: 2024-10-08
-- Purpose: Enable fast tenant validation during availability calculations
-- =====================================================
...
-- Index for availability eligibility queries (ACTIVE or TRIAL)
CREATE INDEX idx_tenant_cache_availability_eligible ON tenant_cache(status)
    WHERE (status IN ('ACTIVE', 'TRIAL')) AND deleted_at IS NULL;

-- Add comments
COMMENT ON TABLE tenant_cache IS 'Local read-only cache of tenant data for Availability Calculator - ensures fast tenant validation';
COMMENT ON COLUMN tenant_cache.id IS 'Tenant UUID - same as in master Tenant Service';
COMMENT ON COLUMN tenant_cache.status IS 'Tenant status - only ACTIVE and TRIAL can calculate availability';
COMMENT ON COLUMN tenant_cache.last_synced_at IS 'Timestamp when this cache entry was last updated from Kafka event';
COMMENT ON COLUMN tenant_cache.deleted_at IS 'Soft delete timestamp - blocks availability calculations when set';
