...
/**
 * TenantCache Repository - Data access for local tenant cache in Payment Processor
 *
 * CRITICAL: This repository is essential for financial data isolation.
 * All payment queries MUST validate tenant status before processing.
 *
...
public interface TenantCacheRepository extends JpaRepository<TenantCache, UUID> {
    ...
    /**
     * CRITICAL: Find all operational tenants that can process payments
    ...
    /**
     * SECURITY: Count tenants that can process payments
     * Used for monitoring and security validation
     */
    @Query("SELECT COUNT(t) FROM TenantCache t WHERE " +
           "(t.status = 'ACTIVE' OR t.status = 'TRIAL') AND t.deletedAt IS NULL")
    long countPaymentEligible();
}
