...
/**
 * Kafka Consumer Configuration for Tenant Events in Payment Processor
 *
 * CRITICAL: This configuration ensures the payment processor receives
 * real-time tenant status updates for security and financial isolation.
 *
 ...
 *
 * Key Features:
 * - Auto-offset reset to earliest for reliability (no missed events)
 ...
 * - Manual acknowledgment for payment-critical updates
 *
 * Security:
 * - All tenant status changes are immediately reflected
 * - Payment blocking/enabling happens in real-time
 * - No payment can proceed without valid tenant status
 */
@Configuration
@EnableKafka
@Slf4j
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, TenantEvent> tenantEventConsumerFactory() {
        ...
        config.put(JsonDeserializer.TRUSTED_PACKAGES, "com.modernreservation.tenant.commons.events");
        config.put(JsonDeserializer.VALUE_DEFAULT_TYPE, TenantEvent.class.getName());
    ...
    public ConcurrentKafkaListenerContainerFactory<String, TenantEvent> tenantEventKafkaListenerContainerFactory() {
        ...
        log.info("SECURITY: Payment processor will receive real-time tenant status updates");
        return factory;
    }
}
