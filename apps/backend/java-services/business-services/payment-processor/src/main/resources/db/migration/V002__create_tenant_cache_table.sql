...
-- Date: 2024-10-08
-- CRITICAL: Essential for financial data isolation and security
-- =====================================================
...
-- Create indexes for common queries and SECURITY validation
...
-- SECURITY: Index for payment eligibility queries (ACTIVE or TRIAL)
CREATE INDEX idx_tenant_cache_payment_eligible ON tenant_cache(status)
    WHERE (status IN ('ACTIVE', 'TRIAL')) AND deleted_at IS NULL;

-- Add comments
COMMENT ON TABLE tenant_cache IS 'CRITICAL: Local read-only cache of tenant data for Payment Processor - ensures financial data isolation and payment security';
COMMENT ON COLUMN tenant_cache.id IS 'Tenant UUID - same as in master Tenant Service';
COMMENT ON COLUMN tenant_cache.status IS 'SECURITY: Tenant status - only ACTIVE and TRIAL can process payments';
COMMENT ON COLUMN tenant_cache.last_synced_at IS 'Timestamp when this cache entry was last updated from Kafka event';
COMMENT ON COLUMN tenant_cache.deleted_at IS 'SECURITY: Soft delete timestamp - blocks new payments when set';
